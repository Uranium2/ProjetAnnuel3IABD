...
def predict_2D_mlp(
    W, layers, layer_count, inputCountPerSample, XTrain, YTrain, low, up
):
    x1 = []
    x2 = []
    y1 = []
    ...
    for x in range(100 * low, 100 * up):
        for y in range(100 * low, 100 * up):
            ...
            result = predict_mlp_classification(
                W, layers, layer_count, inputCountPerSample, dot
            )
            res = result[1:]
            maxi = max(res)
            if maxi >= 0:
...
def predict_2D_mlp_multi(W, layers, layer_count, inputCountPerSample, XTrain, YTrain):
    x1 = []
    x2 = []
    x3 = []
    y1 = []
...
def predict_2D_RBF(W, XTrain, YTrain, inputCountPerSample, gamma, sampleCount, low, up):
