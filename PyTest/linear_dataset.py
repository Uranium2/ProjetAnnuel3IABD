...
if __name__ == "__main__":
    ...
    def fit_save_classif(img_per_folder, h, w, alpha, epochs, prefix):
        ...
        fit_classification_rosenblatt_rule(
            W_FPS, XTrain, sampleCount, inputCountPerSample, YTrain_FPS, alpha, epochs
        )
        fit_classification_rosenblatt_rule(
            W_MOBA, XTrain, sampleCount, inputCountPerSample, YTrain_MOBA, alpha, epochs
        )
        fit_classification_rosenblatt_rule(
            W_RTS, XTrain, sampleCount, inputCountPerSample, YTrain_RTS, alpha, epochs
        )
    ...
    def load_predict_classif_stat(img_per_folder, h, w, pathFPS, pathMOBA, pathRTS):
        ...
        for img in range(img_per_folder * 3):
            ...
            Ypredict_MOBA.append(
                predict_regression(WMOBA, Xpredict, inputCountPerSample)
            )
        ...
        for y in range(len(Ypredict_FPS)):
            if Ypredict_FPS[y] > Ypredict_MOBA[y] and Ypredict_FPS[y] > Ypredict_RTS[y]:
                result.append(0)
            if (
                Ypredict_MOBA[y] > Ypredict_FPS[y]
                and Ypredict_MOBA[y] > Ypredict_RTS[y]
            ):
                result.append(1)
        ...
        for i in range(img_per_folder):
            if result[i] == 0 and i < img_per_folder:
                ...
                result[i + img_per_folder] == 1
                and i > img_per_folder
                and i < 2 * img_per_folder
            ):
                stat.append(True)
            elif (
                result[i + (2 * img_per_folder)] == 2
                and i > 2 * img_per_folder
                and i < 3 * img_per_folder
            ):
                stat.append(True)
            else:
                stat.append(False)
    ...
    load_predict_classif_stat(
        60,
        100,
        100,
        "Models\\Linear\\linear_dataset_FPS_rendu3_18_30_1200.model",
