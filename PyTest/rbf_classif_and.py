from dll_load import create_linear_model, fit_classification_rbf
from pretty_print import predict_2D

if __name__ == "__main__":
    sampleCount = 4
    inputCountPerSample = 2
    alpha = 0.02
    epochs = 2000
    gamma = 100
    YTrain = [-1, -1, -1, 1]
    XTrain = [0, 0, 0, 1, 1, 0, 1, 1]

    W = create_linear_model(inputCountPerSample)

    fit_classification_rbf(W, XTrain, sampleCount, inputCountPerSample, YTrain, alpha, epochs, gamma)

    predict_2D(W, inputCountPerSample, XTrain, YTrain, 0, 1)